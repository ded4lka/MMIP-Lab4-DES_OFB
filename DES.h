...
void ClassicDesRound(bitset<32>& L, bitset<32>& R, vector<bitset<48>> keys, int i)		// Один раунд шифрования DES
{
	bitset<32> Ltemp = L;
	L = R;
	R = Ltemp ^ F(R, keys[i]); // XOR с функцией Фейстеля
	int z = i + 1;
	//cout << "Permutation " << z << ": " << "L: " << L << ", R:" << R << endl;
}

bitset<64> encrypt_block(bitset<64> text, vector<bitset<48>> keys)		//Шифрует 64-битный блок данных с помощью алгоритма DES и предоставленного ключа.
{
...
vector<bitset<64>> encrypt_OFB(vector<bitset<64>> plaintexts, bitset<64> iv, const vector<bitset<48>>& keys) {
	vector<bitset<64>> crypted_texts;
	bitset<64> xor_vector = iv;

	for (auto& plaintext : plaintexts) {
		xor_vector = encrypt_block(xor_vector, keys);  // Шифрование вектора инициализации
		std::cout << "XOR IV:" << xor_vector << "\n\n";
		bitset<64> crypted_text = plaintext ^ xor_vector;  // XOR текста с шифрованным вектором
		crypted_texts.push_back(crypted_text);
	}

	return crypted_texts;
}

vector<bitset<64>> decrypt_OFB(vector<bitset<64>>& crypted_texts, bitset<64> iv, const vector<bitset<48>>& keys) {
	
	return encrypt_OFB(crypted_texts, iv, keys);
}